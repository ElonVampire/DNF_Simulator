...
def getequip(roleid, role, k):
    params = {
        'id': k,
        'name': role,
        'version': 0
    }
    if not os.path.exists(f'Equip/{role}/{k}.json'):
        response = requests.post(
            'http://tools-api.colg.cn/api/v1/equip/equips?token=MTU0Nzk3ODM3NH-gkpmzrtuuf3t4lI2spZO80KzgsobSZ7eIdIW5ppdoi6CD2MrU0GiKoXyUjaylk8bNn6M',
            headers=header,
            params=params)
        print(response.text)
        with open(f'Equip/{role}/{k}.json', 'w', encoding='utf8') as f:
            jsdata = json.loads(response.text.encode('utf8'))
            a = json.dumps(jsdata, ensure_ascii=False)
            f.write(a)
        for equimp in jsdata['data']:
            try:
                path = equimp['picture']
...
if __name__ == '__main__':
    ...
    with open('error.txt', 'w', encoding='utf8') as g:
        for i in jsondata['data']:
            for j in i['pros']:
                if not os.path.exists(f'Equip/{j["transfer_name"]}'):
                    os.mkdir(f'Equip/{j["transfer_name"]}')
                # getequip(j['trans_id'], j['transfer_name'], 1)
                for k in range(1, 13):
                    try:
                        getequip(j['trans_id'], j['transfer_name'], k)
                    except Exception as e:
                        import traceback

                        traceback.print_exc()
                        g.write(f"{j['transfer_name']},{ k}\n")
                        time.sleep(1000)

...
def getskill(role):
    ...
    response = requests.post(
        'http://tools-api.colg.cn/api/v1/buff/buffs?token=MTU0Nzk3ODM3NH-gkpmzrtuuf3t4lI2spZO80KzgsobSZ7eIdIW5ppdoi6CD2MrU0GiKoXyUjaylk8bNn6M',
        headers=header,
        params=params)
    skilllist = ['activeSkill', 'tpSkill', 'buffSkill']
    with open(f'skill/{role}.json', 'w') as f:
        jsdata = json.loads(response.text.encode('utf8'))
        a = json.dumps(jsdata, ensure_ascii=False)
        f.write(a.encode('utf8').decode('utf8'))
    for skill in skilllist:
        for i in jsdata["data"][skill]:
            try:
                path = i['picture']
